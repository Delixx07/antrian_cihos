...
<style>
    ...
    .pc-btn{display:block;margin:1.4rem auto 0;background:#5bc47a;color:#fff;border:none;padding:.75rem 2rem;border-radius:10px;font-weight:700;font-size:.95rem;cursor:pointer;letter-spacing:.02em;}
...
@section('content')
    <div class="pc-wrap">
        ...
        <form method="post" action="{{ route('farmasi.set-counter') }}">
            @csrf
            <select name="jenis" required>
                <option value="">Pilih...</option>
                <option value="non_racik" {{ $jenis==='non_racik'?'selected':'' }}>Farmasi Non Racik</option>
                <option value="racik" {{ $jenis==='racik'?'selected':'' }}>Farmasi Racik</option>
            </select>
